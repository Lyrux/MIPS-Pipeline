# MIPS Pipeline
MIPS Pipeline implementation in Verilog including with timing diagrams.

## Assignment Details
- **Developed by:** Christian Ruelas, Cirena Arabit, Jonathan Kwong
- **Course:** ECE 4300 - Computer Architecture
- **Assignment**: MIPS Pipeline

<!--
## Table of Contents
- [Assignment Details](#assignment-details)
- [Instruction Fetch Timing Diagram](#instruction-fetch-timing-diagram)
- [Instruction Decode Timing Diagram](#instruction-decode-timing-diagram)
- [Execute Stage Timing Diagram](#execute-stage-timing-diagram)
  - [Binary Representation](#binary-representation)
  - [Decimal Representation](#decimal-representation)
- [Memory Writeback Stage Timing Diagram](#memory-writeback-stage-timing-diagram)
  - [Decimal Representation](#decimal-representation-1)

## Instruction Fetch Timing Diagram


## Instruction Decode Timing Diagram
-->

## MIPS Pipeline Timing Diagram
### Decimal Representation
![MIPS Pipeline Decimal Timing Diagram](Timing%20Diagrams/Timing%20Diagram%20-%20MIPS%20Pipeline%20(decimal).png)

## Execute Stage Timing Diagram
### Binary Representation
![Execute Binary Timing Diagram](Timing%20Diagrams/Timing%20Diagram%20-%20Execute%20(binary).png)
### Decimal Representation
![Execute Decimal Timing Diagram](Timing%20Diagrams/Timing%20Diagram%20-%20Execute%20(decimal).png)

## Memory Writeback Stage Timing Diagram
### Decimal Representation
![Execute Decimal Timing Diagram](Timing%20Diagrams/Timing%20Diagram%20-%20Memory%20Writeback%20(decimal).png)

